...
연결을 유지하는데 필요한 리소스를 아껴 더 많은 사용자와 통신하기 위해서 입니다.

</details>

### HTTP 메소드 중 GET, POST 차이점에 대해서 설명해주세요

<details>

GET 메소드는 서버에 있는 리소스를 조회할 목적으로 보내고 POST 메소드는 서버에 리소스를 추가하거나 업데이트할 목적으로 사용됩니다.
또한 GET 메소드는 바디 없이 헤더와 url에 데이터를 담아 보내고 POST 메소드는 BODY에도 데이터를 담아서 보낸다는 차이점이 있습니다.
...
도메인이란 논리적 주소인 ip를 사람이 기억하고 사용하기 쉽게 이름을 부여한 것을 말합니다.
...
도메인 주소를 논리적 주소인 ip로 변환하기 위한 시스템입니다.
...
웹 브라우저에 www.naver.com에 치게 되면 먼저 Local DNS에 해당 호스트네임에 대한 IP주소가 있느지 물어보고 없다면 Root DNS 서버 정보를 전달받습니다.
Root DNS 서버에 질의하면 .com 도메인을 관리하는 TLD(Top-Level Domain) 서버 정보를 전달받습니다. 그러면 TLD에게 www.naver.com에 대해 질의를 합니다. 그러면 naver.com 도메인을 관리하는 DNS 서버 정보를 얻게 되고 얻은 DNS 서버에 www.naver.com 주소를 질의하고 얻게 되면 local dns에 Ip 주소를 캐싱하고 ip 주소 정보를 전달하게 됩니다.
...
트래픽을 여러 서버로 분산 시켜 어느 한 서버에 트래픽 부하를 줄이는 트래픽 분산 처리 기술입니다.

</details>

### L4 로드 밸런싱과 L7 로드 밸런싱 차이점에 대해서 설명해주세요

<details>

L4는 네트워크 계층에서 사용되며 IP와 port를 가지고 트래픽을 분산합니다. L7보다 가격이 저렴하며 단순히 IP, Port만 보고 트래픽을 분산시키기 때문에 속도가 빠릅니다.
L7은 애플리케이션 계층에서 사용되며 헤더나 쿠키를 분석하여 트래픽을 분산합니다. L4보다 정교한 라우팅이 가능하며 데이터 분석을 통해 비정상적인 트래픽도 판별할 수 있습니다. 다만 그만큼 리소스를 많이 소모한다는 단점이 있습니다.
...
### HTTP 0.9 버전에 대해서 설명해주세요

<details>

요청을 할 때 메소드와 URI만 한 줄로 보냈고 메소드도 GET 메소드만 존재했습니다.
응답 같은 경우는 심플하게 HTML 문서만을 보냈습니다. 초기에는 버전 정보가 없었고 이후에 구분을 위해 0.9라고 불리게 되었습니다.

</details>

### HTTP 1.0 버전에 대해서 설명해주세요

<details>

POST, HEAD 메소드가 추가되었고 응답에 상태 코드와 상태 메시지가 추가되었습니다.
그리고 메시지에 헤더가 생겨나게 되었고 생겨남에 따라 Content-Type이라는 헤더도 넣을 수 있게 되어 HTML 문서 이외에 다른 파일들도 전송할 수 있게 되었습니다.
그리고 커넥션 하나당 요청 하나랑 응답 하나를 처리해줄 수 있습니다. 매번 새로운 요청이 올 때마다 새로운 연결을 해주어야 했기 때문에 성능 저하가 발생했습니다.

</details>

### HTTP 1.1 버전에 대해서 설명해주세요

<details>

OPTIONS, PUT, DELETE, TRACE 메소드가 추가되었습니다.
persistent connection과 http pipelining을 도입하였습니다. persistent connection을 이용해서 일정 시간 동안 커넥션을 열어두어 계속해서 연결을 생성하는 것을 해소하였습니다. 근데 HTTP 요청 같은 경우 순차적으로 처리해야하기 때문에 하나씩 처리하는데 대기시간이 길어진다는 문제점이 있었습니다. 그래서 pipelining을 이용해서 커넥션은 응답을 기다리지 않고 순차적으로 요청을 연속적으로 계속 보내 그 순서에 맞춰 응답을 받는 방식으로 대기 시간을 줄였습니다. 하지만 pipelining에도 문제점은 있는데 head of line blocking이라고 여러 개의 요청이 있을 때 순서대로 응답을 한다고 했는데 앞에 있는 요청이 응답으로 만들어지는 데 오래 걸린다면 위에도 다 같이 기다려야 한다는 문제점이 있고 헤더의 구조의 중복이 발생할 수 있습니다.

</details>

### HTTP 2.0 버전에 대해서 설명해주세요

<details>

기존 HTTP 1.x 버전의 성능 향상에 초점을 맞춰 나온 프로토콜입니다.
HTTP 메시지 전송 방식에 변화가 생겼습니다. 바이너리 프레이밍 계층이 추가되었고 이 계층에서 기존에 텍스트 형태로 보냈던 걸 바이너리로 파싱하여 보냄으로써 전송 속도와 오류 발생 가능성을 낮추었습니다.
Multyplexed Streams를 이용해서 한 개의 커넥션에 여러 개의 메시지를 주고받을 수 있게 되었습니다. 순차적으로 보낼 필요 없이 조각을 조합하여 사용이 가능해졌기에 Head of line blocking이 해소되었습니다.
Stream Prioritization을 이용하여 리소스 간 우선순위를 설정하는 것이 가능해졌습니다. server push 기능이 생겨 클라이언트가 요청하지 않은 리소스를 서버에서 보내줄 수 있는 기능이 생겼습니다. 또한 헤더 안에 중복이 있었는데 중복을 압축시키는 방법을 알아내 헤더의 중복을 줄일 수 있게 되었습니다.

</details>

### keep-alive에 대해서 설명해주세요

<details>

HTTP 프로토콜 특징 중 하나인 비연결성의 문제점을 해결하기 위해 나온 것입니다.
한 번의 요청과 응답으로 연결을 해제하게 되는데 실제 하나의 페이지를 로딩할 때 한번이 아니라 여러 번의 요청을 보내게 되는데 그때마다 세션을 연결했다가 해제하기에는 불필요한 리소스가 많이 발생하게 됩니다.
그래서 이를 해결 하기 위해 keep-alive를 connection 헤더에 담아서 보내게 되면 일정 시간 동안 세션을 유지하도록 만들어 계속해서 세션을 연결, 해제로 인한 오버헤드가 발생하지 않도록 합니다.
...
HTTP 프로토콜 특징 중 하나인 무상태의 문제점을 해결하기 위해 나온 것입니다.
연결과 해제가 끝나게 되면 서로에 대해서 아무 상태도 가지지 않게 되는데 이렇게 되면 매번 로그인을 해야 되거나 장바구니가 담기지 않는 현상이 생기므로 상태를 저장하기 위해서 나온 방법입니다.

</details>

### 쿠키와 세션에 차이점에 대해서 설명해주세요

<details>

이 둘의 차이점은 저장 위치, 저장 위치에 따른 보안, 속도, 만료 시점이 있습니다.
저장 위치는 쿠키는 클라이언트 그리고 세션은 서버 메모리에 저장합니다.
저장 위치에 따른 보안은 쿠키가 클라이언트에 있기 때문에 서버에 있는 세션에 비해 보안이 취약할 수 있습니다.
속도는 쿠키가 바로 참조할 수 있지만 세션은 쿠키에 담겨 있는 세션 아이디를 통해서 한번 더 참조해야 하기 때문에 속도가 쿠키에 비해 좀 더 느립니다.
만료 시점은 세션은 브라우저 종료 시에 바로 삭제되지만, 쿠키는 브라우저를 종료해도 만료 기간이 지나지 않았으면 사라지지 않습니다.

</details>

### Web Server, Web Application Server의 차이점에 대해서 설명해주세요

<details>

웹 서버는 정적인 콘텐츠를 제공하는 서버이고 웹 애플리케이션 서버는 디비나 비즈니스 로직 처리가 필요한 동적인 콘텐츠를 제공하기 위해 만들어진 서버입니다.
웹 서버는 Nginx, Apache가 대표적으로 있고 웹 어플리케이션 서버는 Tomcat이 있습니다.

</details>

### CORS(Cross Origin Resource Sharing)에 대해서 설명해주세요

<details>

서버에서 허용하는 호스트만 데이터 접근을 허용할 수 있게 해주는 것을 말합니다.
CORS 동작에는 단순 요청과 프리플라이트 요청 방식이 있습니다.
단순 요청은 바로 서버에게 요청을 보내는 방법입니다. 처음부터 헤더에 Origin에 호스트값을 담아서 보내고 서버는 응답과 함께 access-control-allow-origin 헤더를 담아 보내서 브라우저가 이 둘을 비교하는 방식입니다.
프리플라이트 방식은 요청을 보내기 전에 미리 예비 요청으로 Options 메소드로 요청을 보내 확인을 하고 본 요청을 보내는 방식입니다.
...
REST는 서버에 있는 각각의 리소스들을 uri로 표현 및 구분을 하고 GET, POST와 같은 메소드를 통해서 해야 할 작업의 목표를 나타내고 자원의 상태를 주고받는 것을 REST라고 합니다.
...
해커가 악의적인 스크립트를 삽입하여 사용자 의지와는 무관하게 공격자가 의도한 행위를 하도록 만드는 것을 말합니다. 예를 들어 어떤 게시글을 등록, 수정, 삭제하는 요청을 보내도록 만드는 공격이다.

</details>

### XSS(Cross site scripting)에 대해서 설명해주세요

<details>

악의적인 스크립트를 사이트에 삽입하여 사용자들의 쿠키를 탈취하거나 리다이렉트 시키는 것을 말합니다.
대표적으로 stored xss, reflected xss, dom xss가 있고 통상 부르는 xss는 stored xss 라고 합니다.
stored xss는 공격자가 삽입한 스크립트가 데이터베이스에 저장이 되어 지속해서 서비스에 노출이 되어 악성 스크립트가 노출되는 것을 말합니다.

</details>

### XSS 취약점 막는 방법을 알려주세요

<details>

innerHTML 사용을 자제하고 innerText, textContent를 사용하면 스크립트가 주입되지 않습니다.
그리고 쿠키옵션에 httponly 옵션을 넣게 되면 스크립트로 인해서 쿠키의 접근을 차단할 수 있습니다.
또는 특수 문자들을 다른 문자로 치환해서 사용하는 방법도 있습니다.
...
OAuth는 외부 서비스에서 인증을 가능하게 하고 외부 서비스의 API를 이용할 수 있게 해주는 것을 말합니다.
예로 해당 사이트의 별도의 회원가입 절차 필요 없이 사이트에서 제공하는 플랫폼의 아이디만 있으면 서비스를 이용할 수 있는 것을 말합니다.
